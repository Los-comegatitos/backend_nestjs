...
export class UserService {
  ...
  async create(dto: CreateUserDto, requesterRole?: Role) {
    ...
    if (isNaN(birthDate.getTime())) throw new ConflictException('Invalid birthDate');

    const age = new Date(Date.now() - birthDate.getTime()).getUTCFullYear() - 1970;
    if (age < 18) throw new ConflictException('User must be at least 18 years old');


    const emailExists = await this.userRepo.findOne({ where: { email: dto.email } });
    if (emailExists) throw new ConflictException('Email already registered');

    const phoneExists = await this.userRepo.findOne({ where: { telephone: dto.telephone } });
    if (phoneExists) throw new ConflictException('Telephone already registered');
  ...
  async findById(id: number) {
    const user = await this.userRepo.findOne({ where: { id }, relations: ['typeuser'] });
  ...
  async findByEmail(email: string): Promise<User> {
    const user = await this.userRepo.findOne({ where: { email }, relations: ['typeuser'] });
    if (!user) throw new NotFoundException('User not found');
    return user;
  }
  ...
  async update(id: number, dto: UpdateUserDto) {
    ...
    const updated = await this.userRepo.findOne({ where: { id }, relations: ['typeuser'] });
